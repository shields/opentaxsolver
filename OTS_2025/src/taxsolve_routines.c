...
/* 									*/
/* Copyright (C) 2003-2025 - Aston Roberts				*/
/* 									*/
...
/* 02111-1307 USA							*/
/* 									*/
/* Aston Roberts 1-1-2025	aston_roberts@yahoo.com			*/
/************************************************************************/
...
{ /* Returns 1 on success. 0 on failure. */
 ...
 int std_days[13]={0,31,28,31,30,31,30,31,31,30,31,30,31};  /* Array of Days in each month */
  /* Expect month-day-year as in: 3-3-01, Feb 3, 2025, or 3/3/2025, etc. */
 owrd = strdup( datestr);
...
//============================================
//====== Import Return Data Definitions ======
//============================================
...
// Offset and length to a token within a line of text
typedef struct LINE_TOKEN_T {
    int start;  // -1 = invalid
    int len;    // 0 length is allowable, and expected at the beginning of each line 
} LINE_TOKEN, *P_LINE_TOKEN;

// Import error codes
typedef enum {
    IMPORT_ERR_SUCCESS, // No error
    IMPORT_ERR_FILE_NOT_FOUND,
    IMPORT_ERR_LINE_TOO_LONG,
    IMPORT_ERR_DOUBLE_VAL_TOO_LONG,
    IMPORT_ERR_DOUBLE_CONVERT,
    IMPORT_ERR_ALLOC
} IMPORT_ERR;

// Return status for import, including error code, line number,
// description, and a string and double value for display.
typedef struct IMPORT_STATUS_T {
    IMPORT_ERR err;
    int line_num;
    double value;
    char *string;
...
// Returns the line length, not including null, as with strlen()
// Returns -1 for attempt to read past the max available line_buffer_length,
// which includes the termination '\0'.  This indicates that the line length is
// too long for the max available line buffer size, and the data in 'line' is
// only a partial line.
int ImportLine( FILE *infile, char *line, int line_buffer_length )
{
    char ch;
    int j = 0;
    do { 
        ch = getc(infile);
        if (j < line_buffer_length - 1) {
            line[j++] = ch;
        } else {
            // Null terminate the line and return error, as the line buffer overflowed
            line[j] = '\0';
            return -1;
        }
    } while ((!feof(infile)) && (ch != '\n'));
    // On exiting the while loop, j is pointing to the next
    // offset after the last character.

    // Remove a trailing \r if at the end of the line.
    if ((j >= 2) && (line[j-2] == '\r')) 
        j--;

    // Terminate the line, and return the string length.
    line[j - 1] = '\0';
    return j - 1;
}
...
{ /* Convert slashes in file name based on machine type. */
  char *ptr;
 #ifdef __MINGW32__
  char slash_search='/', slash_replace='\\';
 #else
  char slash_search='\\', slash_replace='/';
 #endif
  ptr = strchr( fname, slash_search );
  while (ptr)
   {
    ptr[0] = slash_replace;
    ptr = strchr( fname, slash_search );
...
// Search for the next token in a line
// Both line and delim must be null terminated strings
// ptoken is modified to point to the next token found
// Returns 0 if successful, -1 on bounds error in ptoken
int NextToken(char *line, char *delim, P_LINE_TOKEN ptoken) 
{
    int line_length = strlen(line);

    // Skip past current token; at start of line 'len' = 0
    int i = ptoken->len;

    // printf("NextToken: line_length: %d, ptoken->start: %d, ptoken->len: %d\n", line_length, ptoken->start, ptoken->len);
    // Check for end of line
    if (i == line_length) {
        // If at end, don't modify the ptoken->start value, and set ptoken->len = 0
        ptoken->len = 0;
        return 0;
    }

    // Check for beyond end of line
    if (i > line_length)
        return -1;

    // Set to invalid
    int next_token_start = -1;
    int next_token_len = 0;

    // Start searching for non-delimiter characters
    while (i < line_length) {
        ...
        int at_end = (i == (line_length - 1));

        // Found the start
        if (!delim_found && (next_token_start == -1)) {
            next_token_start = i;
        }

        // If a delimiter has been found, and a start,
        // the previous character was the end of this token.
        // Set length and stop searching
        if ((delim_found && (next_token_start != -1)))  {
            next_token_len = (i - next_token_start);
            break;
        }

        // If not a delimiter, and at end, set length
        // In this case i is the last character, not one after
        // the last character
        if (!delim_found && at_end) {
            // Set length and stop searching
            next_token_len = i - next_token_start + 1;
            break;
        }

        ++i;    // Next character in line
    }

    // Set the return token values, and return success status
    // If start was never found, reset to beginning of line;
    // in these conditions length will be set to 0
    if (next_token_start != -1) {
        ptoken->start = next_token_start;
    } else {
        ptoken->start = 0;
    }

    ptoken->len = next_token_len;
    return 0;
}

// Create a duplicate string from a token
// The 'token' char pointer is to an embedded string, and so it is NOT null
// terminated: size is only the number of non-null characters in the string, as
// returned by strlen
char *ImportTokenDup(const char *token, size_t size) {
...
// Debug function: print a token within a line, and display a message
void ImportPrintToken(int line, char *msg, char *linebuf, P_LINE_TOKEN ptoken) {
    int i;
    printf("line: %d, start: %d, len: %d, %s = '", line, ptoken->start, ptoken->len, msg);
    for (i = 0; i < ptoken->len; ++i) {
        printf("%c", *(linebuf + ptoken->start + i));
    }
    printf("'\n");
}

// Print import status to output file
void ImportPrintStatus(FILE *outfile, char *msg, IMPORT_STATUS stat) {
    char *err_name;

    err_name = "";
    switch (stat.err) {
        case IMPORT_ERR_SUCCESS:
            err_name = "IMPORT_ERR_SUCCESS";
            break;
        case IMPORT_ERR_FILE_NOT_FOUND:
            err_name = "IMPORT_ERR_FILE_NOT_FOUND";
            break;
        case IMPORT_ERR_LINE_TOO_LONG:
            err_name = "IMPORT_ERR_LINE_TOO_LONG";
            break;
        case IMPORT_ERR_DOUBLE_VAL_TOO_LONG:
            err_name = "IMPORT_ERR_DOUBLE_VAL_TOO_LONG";
            break;
        case IMPORT_ERR_DOUBLE_CONVERT:
            err_name = "IMPORT_ERR_DOUBLE_CONVERT";
            break;
        case IMPORT_ERR_ALLOC:
            err_name = "IMPORT_ERR_ALLOC";
            break;
    }
    
    fprintf(outfile, "ERROR: '%s' import: %s at line: %d\n", msg, err_name, stat.line_num ); 
    
    if (strlen(stat.string)) {
        fprintf(outfile, "%s: '%s'\n", stat.desc, stat.string);
    } else {
        fprintf(outfile, "%s: %5.2f\n", stat.desc, stat.value);
    }
}
...
//====== Import Return Data ======
//================================
#define IMPORT_LINE_BUFFER_LEN 2048
#define IMPORT_DOUBLE_BUFFER_LEN 128 
...
IMPORT_STATUS ImportReturnData(char *return_filename, P_FORM_IMPORT_DEF p_form_imp_def, int num_imp_defs)
{
    ...
    char double_str[IMPORT_DOUBLE_BUFFER_LEN];
    ...
    if (infile==0) {
        ret_stat.err = IMPORT_ERR_FILE_NOT_FOUND;
        ret_stat.string = return_filename;
        ret_stat.desc = "Could not open tax return file for import";
        return ret_stat; 
    }

    int import_line_status;
    do {
        import_line_status = ImportLine(infile, iline, IMPORT_LINE_BUFFER_LEN);
        // Scan the first token of each line.
        // Check for buffer overflow
        if (import_line_status == -1) {
            fclose( infile );
            ret_stat.err = IMPORT_ERR_LINE_TOO_LONG;
            ret_stat.desc = "Line is too long for buffer size";
            ret_stat.value = IMPORT_LINE_BUFFER_LEN;
            return ret_stat; 
        }
        ...
        lt.start = 0;
        lt.len = 0;
        // At beginning of line, bounds error cannot occur in NextToken,
        // so return value does not need to be checked.
        NextToken(iline, delimit, &lt);

        // If no token found, go to next line
        if (lt.len == 0) {
            ++ret_stat.line_num;
            continue;
        }
...
#ifdef IMPORT_DEBUG
        ImportPrintToken(ret_stat.line_num, "Label", iline, &lt);
#endif

        // Pointer to label within line buffer
        char *label;
        int label_len;
        label = iline + lt.start;
        label_len = lt.len;
        for (d = 0; d < num_imp_defs; ++d) {
            P_FORM_IMPORT_DEF pd = p_form_imp_def + d;
            if (strncmp(label, pd->field_name, label_len) == 0 &&
                strlen(pd->field_name) == label_len) {
                // Found a matching label
                // Get the next token (string or numeric value)
                if (NextToken(iline, delimit, &lt) != 0) {
                    fclose( infile );
                    printf("ERROR: Internal token parsing error in form data import at line: %d, start: %d, len: %d\n", ret_stat.line_num, lt.start, lt.len);
                    exit(1);
                }

                // Only process the value token if one was found
                if (lt.len > 0) {
                    // Get double value
                    if (pd->p_field_val != NULL)  {
                        if (lt.len > (IMPORT_DOUBLE_BUFFER_LEN - 1)) {
                            fclose( infile );
                            ret_stat.err = IMPORT_ERR_DOUBLE_VAL_TOO_LONG;
                            ret_stat.desc = "Numeric value string is too long for buffer size";
                            ret_stat.value = IMPORT_DOUBLE_BUFFER_LEN;
                            return ret_stat;
                        }

#ifdef IMPORT_DEBUG
                        ImportPrintToken(ret_stat.line_num, "========== Value", iline, &lt);
#endif

                        // Copy the token to the double_str buffer
                        strncpy(double_str, iline + lt.start, lt.len);
                        // Add null terminator
                        *(double_str + lt.len) = '\0';
                        if (sscanf(double_str, "%lf", pd->p_field_val) != 1) {
                            fclose( infile );
                            ret_stat.err = IMPORT_ERR_DOUBLE_CONVERT;
                            ret_stat.desc = "Value conversion failed for";
                            ret_stat.string = double_str;
                            return ret_stat;
                        }
                    }
                    ...
                    if (pd->p_field_string != NULL) {
                        // Because a space is both a delimiter, and a valid part of the string,
                        // the token length is adjusted to include the rest of the line.
                        int line_length = strlen(iline);
                        lt.len = line_length - lt.start;
                        // Trim any trailing spaces
                        while (lt.len > 0 && *(iline + lt.start + lt.len - 1) == ' ') {
                            --lt.len;
                        }
#ifdef IMPORT_DEBUG
                        ...
                        if (str_result == NULL) {
                            fclose( infile );
                            ret_stat.err = IMPORT_ERR_ALLOC;
                            ret_stat.desc = "Could not allocate memory for string result of length";
                            ret_stat.value = lt.len;
                            return ret_stat;
                        }
                        *pd->p_field_string = str_result;
                    }
                }
