/************************************************************************/
/* TaxSolve_usa_fed1040_2026.c - OpenTaxSolver for USFed1040 		*/
/* Copyright (C) 2026 - Aston Roberts					*/
/* 									*/
/* Tax Solver for US Fedral 1040 Income Tax return for 2026 Tax Year.	*/
/* 									*/
/* OTS Project Home Page and Updates:  					*/
/*		http://opentaxsolver.sourceforge.com/			*/
/* 									*/
/* Compile:   cc taxsolve_US_1040_2026.c -o taxsolve_US_1040_2026       */
/* Run:       ./taxsolve_US_1040_2026  Fed1040_2026.txt                 */
/* 									*/
...
double L13b=0.0;		/* Additional deductions from Schedule 1-A. */
...
char f8960_filename[MAXSTRLEN] = "";
			...
			/* Following values taken from 1040-Instructions pg 80. */	/* Updated for 2026. */
double brkpt[4][9]={
		{ 0.0,  12400.0,  50400.0, 105700.0, 201775.0, 256225.0, 640600.0, 9e19 },  /* Single */
		{ 0.0,  24800.0, 100800.0, 211400.0, 403550.0, 512450.0, 768700.0, 9e19 },  /* Married, filing jointly. */
		{ 0.0,  12400.0,  50400.0, 105700.0, 201775.0, 256225.0, 384350.0, 9e19 },  /* Married, filing separate. */
		{ 0.0,  17700.0,  67450.0, 105700.0, 201775.0, 256200.0, 640600.0, 9e19 },  /* Head of Household. */
		     };
  double txrt[4][9] ={
...
/*  From page 38 of main instructions (i1040gi.pdf).		   */
/*-----------------------------------------------------------------*/
void capgains_qualdividends_worksheets( int status )			/* Updated for 2026. */
{
 int j;
 ...
 if (Do_SchedD)
  ...
  qcgws[3] = L[7];								// CHECK THIS FOR 2026 ?????????????
 qcgws[4] = qcgws[2] + qcgws[3];
 qcgws[5] = NotLessThanZero( qcgws[1] - qcgws[4] );
 switch (status)
  {
   case SINGLE: case MARRIED_FILING_SEPARAT: qcgws[6] = 49450.0; break;
   case MARRIED_FILING_JOINTLY: case WIDOW:  qcgws[6] = 98900.0; break;
   case HEAD_OF_HOUSEHOLD: 		     qcgws[6] = 66250.0; break;
  }
 ...
 switch (status)
  {
   case SINGLE:  			      qcgws[13] = 545500.0;  break;
   case MARRIED_FILING_SEPARAT:	      	      qcgws[13] = 306750.0;  break;
   case MARRIED_FILING_JOINTLY: case WIDOW:   qcgws[13] = 613700.0;  break;
   case HEAD_OF_HOUSEHOLD: 		      qcgws[13] = 579650.0;  break;
  }
...
/*----------------------------------------------------------------------------------------------*/
double form6251_AlternativeMinimumTax( int itemized )						/* Updated for 2026. */
{
 ...
 amtws1a = L[14] - sched1a_l38;
 amtws[1] = L[11] - amtws1a;
 ...
 if ((status == MARRIED_FILING_SEPARAT) && (amtws[4] > 920600.0))
  {
   if (amtws[4] > 1201000.0)
    amtws[4] = amtws[4] + 70100.0;
   else
    amtws[4] = amtws[4] + 0.25 * (amtws[4] - 920600.0);
  }
 ...
 switch (status)
  {
	...
	thresholdA = 640600.0;		/* Phase-out. */
	thresholdB = 1001000.0;
	thresholdC = 244600.0;
	offsetA = 4782.0;
	amt_exmption = 90100.0;
	break;
     case MARRIED_FILING_JOINTLY: case WIDOW:
	thresholdA = 1281200.0;
	thresholdB = 1841600.0;
	thresholdC = 244600.0;
	offsetA = 4782.0;
	amt_exmption = 140200.0;
	break;
 ...
 if (amtws[6] > 0.0)
  { /* AMT Lines 7 through 9, */
	...
	amtws[20] = NotLessThanZero( L[15] );
       ...
       switch (status)
	{
	   case SINGLE:  			      amtws[25] = 533400.0;  break;
	   case MARRIED_FILING_SEPARAT:	      	      amtws[25] = 300000.0;  break;
	   case MARRIED_FILING_JOINTLY: case WIDOW:   amtws[25] = 600050.0;  break;
	   case HEAD_OF_HOUSEHOLD: 		      amtws[25] = 566700.0;  break;
	   default:  printf("Status %d not handled.\n", status);  exit(1); 
	}
       amtws[26] = amtws[21];
       if (Do_QDCGTW)
...
void CapitalLossCarryOverWorksheet( char *fedlogfile, struct FedReturnData *LastYearsReturn )	/* Updated for 2026. */
...
/*									*/
/************************************************************************/
void get_cap_gains()		/* This is Schedule-D. */			/* Updated for 2026. */
{
 ...
 /* New (as of 2026) separate Cap-Gains for "Digital-Assets" (DA) section. */
 ...
 GetLine( "D19", &SchedD[19] );	    /* Amount, if any, from line 18 of Unrecaptured Section 1250 Gain Worksheet. */
 GetLine( "Collectibles", &collectibles_gains );	/* Gains or Losses from Collectibles. (Usually zero.) */
 showline_wlabelnz( "Collectibles_Gains", collectibles_gains );
...
/*--------------------------------------------------------*/
void sched_D_tax_worksheet( int status )			/* Updated for 2026. */
{
 double ws[100];
 int k;
 ...
 switch (status) 
  { case SINGLE: case MARRIED_FILING_SEPARAT: ws[15] = 48350.0; break;
    case MARRIED_FILING_JOINTLY: case WIDOW:  ws[15] = 96700.0; break;
    case HEAD_OF_HOUSEHOLD:      	      ws[15] = 64750.0; break;
  }
 ws[16] = smallerof( ws[1], ws[15] );
 ws[17] = smallerof( ws[14], ws[16] );
 ws[18] = NotLessThanZero( ws[1] - ws[10] );
 switch (status) 
  { case SINGLE: case MARRIED_FILING_SEPARAT: ws[19] = smallerof( ws[1], 197390.0 );  break;
    case MARRIED_FILING_JOINTLY: case WIDOW:  ws[19] = smallerof( ws[1], 394600.0 );  break;
    case HEAD_OF_HOUSEHOLD:      	      ws[19] = smallerof( ws[1], 197300.0 );  break;
  }
 ...
 if (ws[1] != ws[16])
  { /*lines23-43*/
   ...
   switch (status) 
    { case SINGLE: 			ws[26] = 533400.0;  break;
      case MARRIED_FILING_SEPARAT: 	ws[26] = 300000.0;  break;
      case MARRIED_FILING_JOINTLY: 
      case WIDOW:  			ws[26] = 600050.0;  break;
      case HEAD_OF_HOUSEHOLD:		ws[26] = 566700.0;  break;
    }
...
/*-------------------------------------------------------------*/
void SocSec_Worksheet()							/* Updated for 2026. */
{	/* Depends on Sched1, lines 11-25, and 1040 lines 1-8. Sets L[6]. */
 ...
 if (status == MARRIED_FILING_JOINTLY)
  ws[10] = 12000.0;      						/* Updated for 2026. */
 else
  ws[10] = 9000.0;
...
/* Main									*/
/*----------------------------------------------------------------------*/
int main( int argc, char *argv[] )						/* Updated for 2026. */
{
 ...
 /* Expect  US-Fed-1040 lines, something like:
	Title:  Federal 1040 2026 Return
	L1		{Wages}
	L2b		{Interest}
	L3b		{Dividends}
 */
 ...
 check_form_version( word, "Title:  US Federal 1040 Tax Form - 2026" );

 #if (0)
   add_pdf_markup( "NotReady", 1, 240, 40, 17, 1, 1.0, 0, 0, "\"This program is NOT ready for 2026.\"" );
  #ifdef microsoft
   system( "start bin\\notify_popup -delay 3 -expire 10 \"Warning: This program is NOT ready for 2026.\"" );
  #else
   system( "bin/notify_popup -delay 3 -expire 10 \"Warning: This program is NOT ready for 2026.\" &" );
  #endif
#endif
 ...
 get_parameter( infile, 's', word, "You_65+Over?" );	/* Were you born before January 2, 1962 ? (Y/N) */
 ...
 get_parameter( infile, 's', word, "Spouse_65+Over?" );	/* Was Spouse born before January 2, 1962 ? (Y/N) */
 ...
 GetYesNoSL( "CkHomeInUS", &j );	/* Was your main home in USA for more than half the year? */
 if (j)
  fprintf(outfile,"CkHomeInUS X\n");

 GetYesNoSL( "VirtCurr?", &j  );	/* During the year, did you have Virtual Currency? (Y/N) */
 if (j == 0)
  fprintf(outfile,"ChkVirtNo X\n");
 else
  fprintf(outfile,"ChkVirtYes X\n");

 GetYesNoSL( "CkSepLivedApart", &j );	/* If status is MFS or HoH, and lived apart for last 6-months? */
 if ((j) && ((status==MARRIED_FILING_SEPARAT) || (status==HEAD_OF_HOUSEHOLD)))
  fprintf(outfile,"CkSepLivedApart X\n");
 ...
 GetTextLineF( "L1h_type:" );	/* Type of other earned income. */
 ...
 GetYesNoSL( "CkL3c1", &j );	/* Were your child's dividends included in L3a ? */
 if (j) fprintf(outfile,"CkL3c1 X\n");

 GetLineF( "L3b", &L[3] );	/* Ordinary Dividends. (Sched-B) */
 GetYesNoSL( "CkL3c2", &j );	/* Were your child's dividends included in L3b ? */
 if (j) fprintf(outfile,"CkL3c2 X\n");

 GetLineF( "L4a", &L4a );	/* IRAs distributions. */
 GetLineF( "L4b", &L[4] );	/* Taxable IRAs distributions. */
 GetYesNoSL( "Rollover", &j );
 if (j)
  fprintf(outfile,"CkL4c1 X\n");
 GetYesNoSL( "QCD", &j );		/* Qualified Charitable Distribution ? (answer: yes, no) */
 if (j)
  {
   // fprintf(outfile,"QCD = QCD\n");	/* No longer required as of 2026. */
 ...
 GetYesNoSL( "Rollover", &j );
 if (j) fprintf(outfile,"CkL5c1 X\n");
 GetYesNoSL( "PSO", &j );
 if (j) fprintf(outfile,"CkL5c2 X\n");
 ...
 L[25] = L25a + L25b + L25c;
 GetLine( "L26", &L[26] );	/* Estimated tax payments for 2026. */ 
 ...
 GetLine( "L30", &L[30] );	/* Refundable adoption credit from Form 8839, line 13. */

 GetLine( "ApplyTo2026", &L[36] ); /* Percent of OverPayment you want applied to your 2026 estimated tax. */
 if ((L[36] < 0.0) || (L[36] > 100.0))
  {
   printf("Error: 'ApplyTo2026' value (%g) out-of-range (0-100).\n", L[36] );
   fprintf(outfile,"Error: 'ApplyTo2026' value (%g) out-of-range (0-100).\n", L[36] );
   L[36] = 0.0;
  }
 ...
 if (StdDedChart_NumBoxesChecked == 0)
  {
   S_STD_DEDUC   = 16100.0;						/* Updated for 2026. */
   MFJ_STD_DEDUC = 32200.0;
   MFS_STD_DEDUC = 16100.0;
   HH_STD_DEDUC  = 24150.0;
  }
 else
  { /* Std. Deduction chart for People who were Born Before January 2, 1962, or Were Blind, pg 35. */
    switch (StdDedChart_NumBoxesChecked)	/* Does not handle if someone claims you or joint-spouse as dependent. */
     {			/* (Qualifying Widow/er has same amounts as MFJ, so not broken into separate variable.) */
      case 1: 
	S_STD_DEDUC   = 18150.0;					/* Updated for 2026. */
	MFJ_STD_DEDUC = 33850.0;
	MFS_STD_DEDUC = 17750.0;
	HH_STD_DEDUC  = 26200.0;
	break;
      case 2: 
	S_STD_DEDUC   = 20200.0;
	MFJ_STD_DEDUC = 35500.0;
	MFS_STD_DEDUC = 19400.0;
	HH_STD_DEDUC  = 28250.0;
	break;
      case 3: 
	MFJ_STD_DEDUC = 37150.0;
	MFS_STD_DEDUC = 21050.0;
	S_STD_DEDUC   =     1.0;	/* Cannot happen, but set to appease compiler. */
	HH_STD_DEDUC  =     1.0;	/* .. */
	break;
      case 4: 
	MFJ_STD_DEDUC = 38800.0;
	MFS_STD_DEDUC = 22700.0;
	S_STD_DEDUC   =     1.0;	/* Cannot happen, but set to appease compiler. */
	HH_STD_DEDUC  =     1.0;	/* .. */
	break;
 ...
 if (status != MARRIED_FILING_SEPARAT)
  loctaxlimit = 40400.0;					/* Updated for 2026. */
 else
  loctaxlimit = 20200.0;
 ...
 GetYesNo( "A18", &ForceItemize );	/* Elect to itemize, even when less than standard deduction. */
 ...
 showline( 10 );
 showline_wlabelmsg( "L11a", L[11], "Adjusted Gross Income (AGI)" );
 showline_wlabel( "L11b", L[11] );
 ...
 switch (status)	/* Check for minimum income to file. (min2file) */		/* Updated for 2026. */
  {			/* Listed in Instructions page-9, in Chart A - For Most People. */
   case SINGLE:  		  if (under65) exemption_threshold = 15700.0;
				  else  exemption_threshold = 17750.0;
	break;
   case MARRIED_FILING_JOINTLY:	  if (under65==2) exemption_threshold = 31500.0;
				  else 
   ...
   case MARRIED_FILING_SEPARAT:  exemption_threshold = 5.0;
	break;
   case HEAD_OF_HOUSEHOLD: 	  if (under65) exemption_threshold = 23625.0;  
				  else  exemption_threshold = 23625.0;
	break;
   case WIDOW:  		  if (under65) exemption_threshold = 31500.0;  
				  else  exemption_threshold = 33100.0;
  }
 ...
 showline( 12 );
 showline_wlabel( "L13a", L[13] );		/* Qualified business income ded., read-in above. */
 showline_wlabel( "L13b", L13b );		/* Additional deductions from Schedule 1-A, Line 38. */
  ...
  { /*OptionalLine*/
   ...
   if (word[0] != '\0')
	...
	if (toupper(word[0]) == 'Y')
     ...
     if (strncmp( labelx,"CkMoreThan4Dep", 14 ) == 0)
      {
	if (toupper(word[0]) == 'Y')
	 fprintf(outfile, "%s X\n", labelx );
      }
