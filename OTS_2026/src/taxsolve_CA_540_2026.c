/************************************************************************/
/* TaxSolve_CA_540_2026.c - California state 540 tax form.		*/
/* Copyright (C) 2026 - Aston Roberts					*/
/* 									*/
/* Compile:   gcc taxsolve_CA_540_2026.c -lm -o taxsolve_CA_540_2026	*/
/* Run:	      ./taxsolve_CA_540_2026  CA_540_2026.txt 			*/
/* 									*/
...
{									/* Updated for 2026. */
 ...
 if ((status==SINGLE) || (status==MARRIED_FILING_SEPARAT))
  {
   if (income <  11408.00)  tax =             0.01 * income;                else
   if (income <  27044.00)  tax =   114.08 +  0.02 * (income -  11408.00);  else
   if (income <  42683.00)  tax =   426.80 +  0.04 * (income -  27044.00);  else
   if (income <  59251.00)  tax =  1052.36 +  0.06 * (income -  42683.00);  else
   if (income <  74884.00)  tax =  2046.44 +  0.08 * (income -  59251.00);  else
   if (income < 382514.00)  tax =  3297.08 + 0.093 * (income -  74884.00);  else
   if (income < 459009.00)  tax = 31906.67 + 0.103 * (income - 382514.00);  else
   if (income < 765025.00)  tax = 39785.66 + 0.113 * (income - 459009.00);
   else                     tax = 74365.47 + 0.123 * (income - 765025.00);
  }
 else
 if ((status==MARRIED_FILING_JOINTLY) || (status==WIDOW))
  {
   if (income <   22816.00)  tax =              0.01 * income;                 else
   if (income <   54088.00)  tax =    228.16 +  0.02 * (income -   22816.00);  else
   if (income <   85366.00)  tax =    853.60 +  0.04 * (income -   54088.00);  else
   if (income <  118502.00)  tax =   2104.72 +  0.06 * (income -   85366.00);  else
   if (income <  149768.00)  tax =   4092.88 +  0.08 * (income -  118502.00);  else
   if (income <  765028.00)  tax =   6594.16 + 0.093 * (income -  149768.00);  else
   if (income <  918018.00)  tax =  63833.34 + 0.103 * (income -  765028.00);  else
   if (income < 1530038.00)  tax =  79593.21 + 0.113 * (income -  918018.00);
   else                      tax = 148751.47 + 0.123 * (income - 1530038.00);
  }
 else
  { /* Head of Household. */
   if (income <   22832.00)  tax =             0.01 * income;                else
   if (income <   54091.00)  tax =   228.32 +  0.02 * (income -  22832.00);  else
   if (income <   69728.00)  tax =   853.50 +  0.04 * (income -  54091.00);  else
   if (income <   86294.00)  tax =  1479.98 +  0.06 * (income -  69728.00);  else
   if (income <  101932.00)  tax =  2473.94 +  0.08 * (income -  86294.00);  else
   if (income <  520215.00)  tax =  3724.98 + 0.093 * (income - 101932.00);  else
   if (income <  624262.00)  tax = 42625.29 + 0.103 * (income - 520215.00);  else
   if (income < 1040427.00)  tax = 53341.13 + 0.113 * (income - 624262.00);
   else                      tax = 100368.77 + 0.123 * (income - 1040427.00);
  }
 return (int)(tax+0.5);
}
...
void Report_bracket_info( double income, int status )
{
 ...
 if ((status==SINGLE) || (status==MARRIED_FILING_SEPARAT))
  {
   if (income <   11408.00)  rate = 0.01;  else
   if (income <   27044.00)  rate = 0.02;  else
   if (income <   42683.00)  rate = 0.04;  else
   if (income <   59251.00)  rate = 0.06;  else
   if (income <   74884.00)  rate = 0.08;  else
   if (income <  382514.00)  rate = 0.093;  else
   if (income <  459009.00)  rate = 0.103;  else
   if (income <  765025.00)  rate = 0.113;  else  rate = 0.123;
  }
 else
 if ((status==MARRIED_FILING_JOINTLY) || (status==WIDOW))
  {
   if (income <   22816.00)  rate = 0.01;  else
   if (income <   54088.00)  rate = 0.02;  else
   if (income <   85366.00)  rate = 0.04;  else
   if (income <  118502.00)  rate = 0.06;  else
   if (income <  149768.00)  rate = 0.08;  else
   if (income <  765028.00)  rate = 0.093;  else
   if (income <  918018.00)  rate = 0.103;  else
   if (income < 1530038.00)  rate = 0.113;  else  rate = 0.123;
  }
 else
  {
   if (income <   22832.00)  rate = 0.01;  else
   if (income <   54091.00)  rate = 0.02;  else
   if (income <   69728.00)  rate = 0.04;  else
   if (income <   86294.00)  rate = 0.06;  else
   if (income <  101932.00)  rate = 0.08;  else
   if (income <  520215.00)  rate = 0.093;  else
   if (income <  624262.00)  rate = 0.103;  else
   if (income < 1040427.00)  rate = 0.113;  else  rate = 0.123;
  }
 ...
 */
 ...
 check_form_version( word, "Title:  CA State Tax Form 540 - 2026" );
 ...
 L[7] = 158.0 * iline7;							/* Updated for 2026. */
 showline(7);
 ...
 L[8] = iline8 * 158.0;							/* Updated for 2026. */
 showline(8);
 ...
 if (status != MARRIED_FILING_SEPARAT)
  sched540part2[5] = smallerof( sched540part2_5d, 40400.0 );	/* Will be Line 5e. */
 else
  sched540part2[5] = smallerof( sched540part2_5d, 20200.0 );
 ...
 switch (status)
  {
   case SINGLE:
   case MARRIED_FILING_SEPARAT:  threshA = 259697.0;	std_ded = 5876.0;   break;	/* Updated for 2026. */
   case HEAD_OF_HOUSEHOLD:       threshA = 389551.0;	std_ded = 11751.0;  break;
   case MARRIED_FILING_JOINTLY:
   case WIDOW:                   threshA = 519399.0;	std_ded = 11751.0;  break;
  }
 if (L[13] > threshA)
 ...
 switch (status)
  {		/* Minimum AGI (Line 17) required to file. */		/* Updated for 2026. */
    case SINGLE:
    case HEAD_OF_HOUSEHOLD:
		if (iline9 == 0)		
		  min2file = 18353.0;		/*Under65*/
		else
		  min2file = 26003.0;		/*65over*/
	  break;
   case MARRIED_FILING_JOINTLY: 
		if (iline9 == 0)		
		  min2file = 36711.0;		/*BothUnder65*/
		else
		if (iline9 == 1)		
		  min2file = 44361.0;		/*OneUnder65*/
		else
		  min2file = 52011.0;		/*Both65over*/
	  break;
   case WIDOW:
		if (iline9 == 0)		
		  min2file = 34186.0;		/*Under65*/
		else
		  min2file = 37878.0;		/*65over*/
	  break;
